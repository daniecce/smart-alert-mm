"""Main screen: BI filters, MM recommendation grid, session-based transfer registry."""
...
from config.constants import MAGAZYNY_CENTRALNE, PLIK_DOSTAWY_BO
...
def generuj_excel_rejestru(lista_transakcji):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        dopasuj_szerokosci_kolumn(worksheet, margines=4, min_szerokosc=15)
...
def wyswietl_ekran_glowny(wyciagnij_rdzen_po_przecinku, convert_df):
    ...
    rekomendacje = generuj_rekomendacje_mm(
        ...
        zakres_dni=zakres_dni, cena_min=cena_min, marza_min=marza_min, popyt_min=popyt_min,
        dni_karencji=dni_karencji,
    ...
    if rekomendacje.empty:
        ...
        raport_grouped = zbuduj_raport_grouped(rekomendacje)
        ...
        try:
            import os
            # 🔴 NAPRAWIONE (bug obecny też w oryginale): ścieżka względna 'Smart_MM' zależała od
            # katalogu roboczego (CWD) w momencie uruchomienia streamlit, więc działała tylko gdy
            # aplikację odpalano z katalogu głównego projektu. Tu używamy tej samej ścieżki
            # bezwzględnej co core/data_loader.py, żeby działało niezależnie od CWD.
            sciezka_bo = os.path.join(FOLDER_DANYCH_ABS, PLIK_DOSTAWY_BO)
            if not os.path.exists(sciezka_bo):
                # 🔴 NAPRAWIONE (literówka obecna też w oryginale): ta gałąź ustawiała kolumnę 'BO',
                # a kod niżej (linia z finalnym wyborem kolumn) oczekuje 'Ilość BO' - w oryginale
                # nigdy się to nie ujawniło, bo plik BO zawsze istniał przy uruchomieniu z roota.
