...
def pobierz_sciezke_folderu():
    sciezka_core = os.path.dirname(os.path.abspath(__file__))
    sciezka_glowna = os.path.dirname(sciezka_core)
    return os.path.join(sciezka_glowna, 'Smart_MM')

FOLDER_DANYCH = pobierz_sciezke_folderu()
...
def zaladuj_i_parsuj_dane():
    ...
    if os.path.exists(sciezka_stany) and os.path.exists(sciezka_sprzedaz) and os.path.exists(sciezka_minmax):
        ...
        str_web.warning(f"⏳ Oczekiwanie na komplet plików w folderze OneDrive... Oczekiwana lokalizacja: `{FOLDER_DANYCH}`")
